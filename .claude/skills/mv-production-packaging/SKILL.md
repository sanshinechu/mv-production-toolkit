...
| 步驟 | 名稱 | 負責 | 時間 | 狀態 |
|------|------|------|------|------|
| 1️⃣ | MV_01：歌詞創作 + SUNO 風格 | Claude | 30-45 分 | ✅ 完成 |
| 2️⃣ | SUNO 音樂生成 | 外部（SUNO） | 30-60 分 | ✅ 完成 |
| 3️⃣ | MV_02：主角設計（女+男） | Claude | 15-30 分 | ✅ 完成 |
| 4️⃣ | MV_03：場景影像提示詞 | Claude | 30-40 分 | ✅ 完成 |
| 5️⃣ | MV_04：九宮格分鏡 | Claude + Codex | 20-30 分 設計 + 生圖 | ✅ 完成 |
| 6️⃣ | MV_05：截圖放大提質 | Claude + Codex | 10-15 分 | ✅ 完成 |
| 7️⃣ | MV_06：影片動態提示詞 | Claude | 20-30 分 | ✅ 完成 |
...
**階段 1：設計階段（Claude Code 主導）**
- MV_01 歌詞創作
- MV_02 角色設計
- MV_03-06 視覺與動態提示詞設計
- 準備完整的提示詞與參考資源

**階段 2：生成與組裝階段（Codex 為主 + Claude 組裝驗證）**
- Codex 使用 `media-generation` skill 生成圖片與視頻
- Claude 用 FFmpeg 組裝、驗證、發布
...
**快速開始**：
- 複製 `.claude/skills/mv-production-packaging/metadata-template.md`
- 改名為 `metadata.md` 放到 `outputs/<MV標題> [Claude]/`
- 根據你的 MV 修改內容即可
...
在 `outputs/<MV標題> [Claude]/` 中建立 README.md，簡述製作過程與觀看指南：
...
**Q: Codex 和 Claude Code 如何分工？**
A: Claude 負責設計與驗證，Codex 負責 AI 生圖生影（media-generation）。FFmpeg 組裝與 YouTube 上傳由 Claude 負責。
