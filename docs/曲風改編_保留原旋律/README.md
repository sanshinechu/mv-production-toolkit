...
## 九之三、AI 人聲貼到 Suno 伴奏：拍點對齊（2026-09-13）

OpenUtau 算出來的人聲放進 Suno 伴奏，聽起來拍子和音準都「差一點」。
量過之後發現兩個根因，腳本 `腳本/snap_ustx_to_backing_beats.py` 一次處理：

```bash
uv run --python 3.11 --with librosa --with soundfile --with pyyaml python 腳本/snap_ustx_to_backing_beats.py \
  原本.ustx suno伴奏.wav 新的.ustx --report 報告.csv --preview 旋律預覽.wav
```

| 根因 | 實測 | 腳本怎麼修 |
|---|---|---|
| **Suno 伴奏速度會飄** | 同一首從 123.8 爬到 126 再掉到 122.3 BPM，套固定 BPM 誤差達 ±150ms；USTX 還設成 125 | 逐拍偵測後平滑，寫成**逐拍速度表**，OpenUtau 的格線就等於伴奏的拍點；字頭吸到 1/8 拍（太遠才 1/16） |
| **逐音摺八度** | 句中出現 12 半音大跳 | 句內大跳才還原，再整句一起移八度進音域 |

驗收（《至少還有你》）：301 個字頭全部在 1/16 格線上、沒有重疊；格線對伴奏鼓點的
最佳偏移是 0ms；字頭平均只移 29ms。

> 🕳️ **句子接縫的八度大跳多半是原曲真的這樣唱，不要「修」。**
> 9 處 70→58 的跳躍到參考主唱原音用 pyin 量過：「放棄→至少」「珍惜→而你」
> 「忘記→就是」「秘密→你掌心」8 處是原曲真實旋律，只有「失去你」的「去」是
> 被錯摺（原唱 70→72，USTX 寫成 60）。所以腳本只動**句內**大跳（`--phrase-gap` 0.15 秒）。

> ⚠️ **OpenUtau 匯入伴奏要先轉 44.1kHz。** 48kHz 音檔加上多段速度變化，
> 播放和匯出會跑掉（[OpenUtau #2264](https://github.com/openutau/OpenUtau/issues/2264)，
> 2026-09 仍未修）。逐拍速度表正好踩中這個條件。

> 🕳️ **試過但不可行：拿參考曲和 Suno 伴奏比對和聲來對時間（chroma DTW）。**
> 找到的位置亂跳，而且跟故意放錯段的對照組分數一樣（0.083 vs 0.089）——
> 搖滾和弦來回重複、破音吉他把和聲糊在一起。要走這條得有當初上傳 Suno 的 guide.wav。

> 🔴 **這支只修「格線」，修不了「歌詞太多」。** 同日使用者聽了指出真正的問題是
> 把原版全詞塞進 186 秒改編版：主歌導歌被壓快 1.3～2.2 倍。先要**逐句聽 Suno 原檔
> 決定每句放哪、刪哪句**，再談對拍。《至少還有你》逐句對位的結果是：Suno 這版只有主歌／
> 預副歌的句子（共 24 句），副歌、橋段、「形影不離」都沒唱。
> 另外 Ria（DiffSinger）照這份對位試唱 19–47 秒，使用者判定**音色本身不好聽、節奏不對**，
> 不可用；下一版音源要換方向（真人唱法＋RVC、Suno 自己唱，或換聲庫，尚未決定）。

`報告.csv` 的 `weak_long_note` 標出跟伴奏和弦明顯不合的長音（排名 12 個音裡後 4 名）。
這個檢查有雜訊，**當作「先聽哪裡」的清單，不是自動修正的依據**。
《至少還有你》標出 38 個，**副歌後的 Bridge 最密集**：Suno 伴奏在那段偏向 B♭ 為中心，
原曲仍是 E♭，很可能是 Suno 自己改了和聲。
