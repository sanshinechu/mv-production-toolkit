...
拿《至少還有你》整套跑過一遍。**失敗的那些比成功的更值得留。**

### ✅ 確認可行

| 做法 | 結果 |
|---|---|
| 合成導引音軌上傳 Suno | **通過指紋閘門**，Cover 正常運作 |
| Cover 生出的搖滾版 | **好聽，旋律認得出是原曲** |
| 導引音軌保真度 | E♭ 大調、123 BPM、音域 D#3~C#6，**四項都跟原曲對得上** |

### ❌ 確認不可行（別再試了）

| 做法 | 結果 | 為什麼 |
|---|---|---|
| 貼《至少還有你》原詞 | 擋 | 不是只有杰威爾嚴，滾石一樣擋 |
| 上傳林憶蓮原唱 | 擋 | 商業母帶，預期內 |
| **上傳 Agroce 的 AI 搖滾版** | **擋** | 意外發現：**Suno 連自己生成過的曲子也建了指紋**。所以「拿別人的 AI 成品當素材」這條路整個不通 |
| 上傳 `.mid` 檔 | 不支援 | Suno 只吃 MP3/WAV/OGG/M4A/FLAC。**MIDI 要先自己渲染成音檔** |

> 🕳️ 上傳 `.mid` 那次畫面上出現「matches an existing recording」，一度讓人以為
> **Suno 比對的是曲子不是錄音**——那會讓整條路線全毀。實際上那是前一次上傳
> 殘留在畫面上的訊息。**紅色橫幅不會自己消失，測下一個之前先把它關掉。**

### 🔴 疊軌方案（原唱人聲 ＋ Suno 新伴奏）實測失敗

這是 9/11 選定的主線，跑完發現**架構上就不成立**，已放棄：

| 症狀 | 根因 |
|---|---|
| 聲音打架、不和諧 | Suno 編伴奏時**看不到人聲**，只能自己猜和弦，猜錯就跟原唱衝突 |
| 技術上沒錯但不好聽 | 原唱是**抒情演唱**，不可能「知道」自己配的是搖滾伴奏 |

**兩個症狀同源**：伴奏和人聲是兩個互不知道對方存在的系統各自產生的。
補和聲只能救第一個，第二個無解。**RVC／seed-vc 也救不了**——
那類工具換的是「音色」（誰的聲音）不是「唱腔」（怎麼唱），
林憶蓮的句法、氣口、力度曲線會原封不動跟過去。

👉 **正解是讓 Suno 重新演唱**：伴奏和人聲出自同一個模型，和聲天生一致、
唱腔由 Style 決定。代價是歌詞必須是自己的。

### 所以最終路線

```
原曲 → Demucs 分離人聲 → Basic Pitch 抽旋律 → 合成導引音軌
     → Suno Cover（Style 寫搖滾現場 ＋ 自己填的新詞）→ 成品
```

**原旋律 ✅　自己的詞　搖滾唱腔 ✅　好聽 ✅**

原詞拿不到。這是用現有工具能達到的最好結果，不是還沒找到方法。
